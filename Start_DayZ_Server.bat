...
start /HIGH "DayZ Server" /MIN "%DayZ_Path%\%EXE_Name%" "-config=%DayZ_Path%\%ServerCFG%" -port=%GamePort% "-bePath=%DayZ_Path%\battleye" "-servermod=" "-mod=" "-profiles=%DayZ_Path%\%Profiles%" -cpuCount=%CPUCores% -doLogs -adminlog -freezecheck
